...
import com.k70mobile.mytasktodo.models.SelectedMembers
...
open class CardListItemAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is MyViewHolder) {
            ...
            if ((context as TaskListActivity).assignedMemberDetailsList.size > 0) {
                val selectedMembersList: ArrayList<SelectedMembers> = ArrayList()

                for (i in context.assignedMemberDetailsList.indices) {
                    for (j in model.assignedTo) {
                        if (context.assignedMemberDetailsList[i].id == j) {
                            val selectedMembers = SelectedMembers(
                                context.assignedMemberDetailsList[i].id,
                                context.assignedMemberDetailsList[i].image
                            )
